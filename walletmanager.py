...
# Constants
PROVISION_NUMBER_FIRST_LIQUIDITY = [i for i in range(1,2)]
...
def capture_logs_periodically(container_names, amount, btc_node, interval=55):
    if shutdown_event.is_set():
        return
    
    for container_name in container_names:
        build_logs_file(container_name)
        parse_address_send_btc(f"whirlpool-sparrow-client/logs/{container_name}.txt")
...
def main(): 
    ...
    capture_logs_periodically(wallet_containers, 0.000045, btc_node)
    time.sleep(300)
